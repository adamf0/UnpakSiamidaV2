...
func TestNewTemplateRenstra_Success(t *testing.T) {
	...
	result := domain.NewTemplateRenstra(
		...
		nil,                 // satuan
		nil,                 // target
		helper.StrPtr("10"), // targetMin
		helper.StrPtr("20"), // targetMax
		tugas,
	)
...
func TestUpdateTemplateRenstra_Success(t *testing.T) {
	prev := domain.NewTemplateRenstra(
		"2025", 1, true, 1, "C", "S1",
		nil, nil, helper.StrPtr("10"), helper.StrPtr("20"), "Unit",
	).Value
	...
	result := domain.UpdateTemplateRenstra(
		...
		nil,                  // satuan
		nil,                  // target
		helper.StrPtr("100"), // targetMin
		helper.StrPtr("200"), // targetMax
		"Unit",
	)
...
func TestUpdateTemplateRenstra_NegativeCases(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			uid:  prev.UUID, target: helper.StrPtr("10"), targetMin: helper.StrPtr("5"), targetMax: nil,
			indikatorID: 1, fakultasUnit: 1,
			expectedErrDesc: domain.InvalidValueTarget().Description,
		},
		{
			name: "Invalid indikatorRenstraID",
			uid:  prev.UUID, target: helper.StrPtr("10"), targetMin: nil, targetMax: nil,
		...
		{
			name: "UUID mismatch",
			uid:  uuid.New(), target: helper.StrPtr("10"), targetMin: nil, targetMax: nil,
			indikatorID: 1, fakultasUnit: 1,
