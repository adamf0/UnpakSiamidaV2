...
import (
	...
	app "UnpakSiamida/modules/templaterenstra/application/GetTemplateRenstra"
	infra "UnpakSiamida/modules/templaterenstra/infrastructure"
)

func TestGetTemplateRenstraByUuid_Success(t *testing.T) {
	db, cleanup := setupTemplateRenstraMySQL(t)
	defer cleanup()

	repo := infra.NewTemplateRenstraRepository(db)
	handler := app.GetTemplateRenstraByUuidQueryHandler{Repo: repo}

	// UUID fix yang kamu tentukan
	fixedUUID := "298923cc-1077-4132-b24d-8fc9b3087ef5"

	q := app.GetTemplateRenstraByUuidQuery{Uuid: fixedUUID}
...
func TestGetTemplateRenstraByUuid_Errors(t *testing.T) {
	db, cleanup := setupTemplateRenstraMySQL(t)
	defer cleanup()
	...
	for _, tt := range tests {
		...
		t.Run(tt.name, func(t *testing.T) {
			q := app.GetTemplateRenstraByUuidQuery{Uuid: tt.uuid}
