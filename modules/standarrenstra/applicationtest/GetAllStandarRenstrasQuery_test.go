...
func TestGetAllStandarRenstrasIntegration(t *testing.T) {
	db, cleanup := setupStandarRenstraMySQL(t)
	defer cleanup()

	// Use GORM wrapper repo constructor
	gormrepo := infra.NewStandarRenstraRepository(db)
	handler := app.GetAllStandarRenstrasQueryHandler{Repo: gormrepo}
	...
	for _, tt := range searchTests {
		t.Run("Search_"+tt.name, func(t *testing.T) {
			...
			q := app.GetAllStandarRenstrasQuery{
				Search:        tt.search,
