...
import (
	...
	app "UnpakSiamida/modules/beritaacara/application/GetBeritaAcara"
	infra "UnpakSiamida/modules/beritaacara/infrastructure"
)

func TestGetBeritaAcaraByUuid_Success(t *testing.T) {
	db, cleanup := setupBeritaAcaraMySQL(t)
	defer cleanup()

	repo := infra.NewBeritaAcaraRepository(db)
	handler := app.GetBeritaAcaraByUuidQueryHandler{Repo: repo}

	// UUID fix yang kamu tentukan
	fixedUUID := "14212231-792f-4935-bb1c-9a38695a4b6b"

	q := app.GetBeritaAcaraByUuidQuery{Uuid: fixedUUID}
...
func TestGetBeritaAcaraByUuid_Errors(t *testing.T) {
	db, cleanup := setupBeritaAcaraMySQL(t)
	defer cleanup()
	...
	for _, tt := range tests {
		...
		t.Run(tt.name, func(t *testing.T) {
			q := app.GetBeritaAcaraByUuidQuery{Uuid: tt.uuid}
